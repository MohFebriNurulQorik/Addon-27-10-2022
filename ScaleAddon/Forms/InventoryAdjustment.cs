...
namespace ScaleAddon.Forms
{
    public partial class InventoryAdjustment : Form
    {
        ...
        private DataTable dtDetailRecipt;
        private DataTable dtDetailIssue;
        ...
        private void loadProcess()
        {
            ...
            this.Text = $"Universal Leaf [{Warehouse.Descr}] - Inventory Adjustment Process [{DocNumber}]";
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    string query = $"select * from InventoryAdjustment where DocumentID = '{DocNumber}' and DocumentDate = '{currentDate.Date.ToString("yyyy-MM-dd")}'";
                    ...
                    if (reader.HasRows)
                    {
                        ...
                        remark.Text = reader.GetValue(6).ToString();
        ...
        private void loadProsess()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                //clear combo
                //cbFarmer.DataSource = null;
                oldProcess.SelectedIndex = -1;
                dtInventory = new DataTable();
                try
                {


                    string query = "select * from ItemAttribute where CodeType = 'PROCESS'";
                    ...
                    da.Fill(dtInventory);
                    string[] arrray = dtInventory.Rows.OfType<DataRow>().Select(k => k[0].ToString() + " | " + k[2].ToString()).ToArray();

                    oldProcess.Items.Clear();
                    oldProcess.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboInv()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    oldInventory.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void resetEntry()
        {
            ...
            switch (tbStatus.Text)
            {
                case "OPEN":
                    ...
                    remark.BackColor = System.Drawing.SystemColors.Info;
                    break;
        ...
        private void loadComboSource()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    oldSource.Items.Clear();
                    oldSource.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboStage()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    //new AutoCompleteBehavior(oldStage);
                    oldStage.Items.Clear();
                    oldStage.Items.AddRange(arrray);
        ...
        private void loadComboForm()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    //new AutoCompleteBehavior(oldForm);
                    oldForm.Items.Clear();
                    oldForm.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboCropYear()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    oldCropYear.Items.Clear();
                    oldCropYear.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboGrade()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                //clear combo
                //cbFarmer.DataSource = null;
                oldGrade.SelectedIndex = -1;

                dtEntryGrade = new DataTable();
                try
                {
                    string query = $"select * from TobaccoGrade where WarehouseID = '{tbWarehouse.Text}'";
                    SqlCommand command = new SqlCommand(query, connection);
                    connection.Open();
                    ...
                    oldGrade.Items.Clear();
                    oldGrade.Items.Add("-NOT GRADED-");
                    oldGrade.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboArea()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    //new AutoCompleteBehavior(oldArea);
                    oldArea.Items.Clear();
                    oldArea.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboStalk()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                //clear combo
                //cbFarmer.DataSource = null;
                oldStalkPosition.SelectedIndex = -1;
                ...
                try
                {
                    ...
                    oldStalkPosition.Items.Clear();
                    oldStalkPosition.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboColor()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    //new AutoCompleteBehavior(oldColor);
                    oldColor.Items.Clear();
                    oldColor.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboFerment()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                //clear combo
                //cbFarmer.DataSource = null;
                oldFermentation.SelectedIndex = -1;
                ...
                try
                {
                    ...
                    //new AutoCompleteBehavior(oldFerment);
                    oldFermentation.Items.Clear();
                    oldFermentation.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
        ...
        private void loadComboLength()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    //new AutoCompleteBehavior(oldLength);
                    oldLength.Items.Clear();
                    oldLength.Items.AddRange(arrray);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.ToString());
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    string query = $@"SELECT *
                                        ...
                                        WHERE
	                                        DocumentID = '{DocNumber}' ORDER BY
	                                        right(LotNbr,5) DESC, Iostock DESC";
        ...
        private void loadDetailIssue()
        {
            //load data untuk grid
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                //create new dt
                dtDetailIssue = new DataTable();
                try
                {
                    //string query = $"SELECT * from BuyingRegistration where RegistrationDate = '{currentDate.ToString()}'";
                    ...
                    string query = $@"SELECT *
                                        FROM
	                                        InventoryAdjustmentDetail
                                        WHERE
	                                        DocumentID = '{DocNumber}' and Iostock = -1";

                    SqlCommand command = new SqlCommand(query, connection);
                    connection.Open();

                    SqlDataAdapter da = new SqlDataAdapter(command);
                    da.Fill(dtDetailIssue);
                    dgvDetail.DataSource = dtDetailIssue;
        ...
        private void loadDetailRecipt()
        {
            //load data untuk grid
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                //create new dt
                dtDetailRecipt = new DataTable();
                try
                {
                    //string query = $"SELECT * from BuyingRegistration where RegistrationDate = '{currentDate.ToString()}'";
                    ...
                    SqlDataAdapter da = new SqlDataAdapter(command);
                    da.Fill(dtDetailRecipt);
                    dgvDetail.DataSource = dtDetailRecipt;
        ...
        private void getDocLastIncrement()
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    string query = "select * from NumberingSetting where NumberingID = 'IA'";
                    SqlCommand command = new SqlCommand(query, connection);
                    connection.Open();
                    SqlDataReader reader = command.ExecuteReader();
        ...
        private void setDocNumber()
        {
            ...
            if (lastIncrementValue >= 0)
            {
                ...
                var docNbr = $"{Math.Abs(FiscalInfo.CurrentFiscalYear) % 100}{Warehouse.WarehouseID}-IA-{currentIncrement.ToString().PadLeft(4, '0')}";
        ...
        private bool checkDocNbrExist(string docNbr)
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    Console.WriteLine(docNbr);
                    string query = $"IF EXISTS ( SELECT * FROM InventoryAdjustment WHERE DocumentID = '{docNbr}' ) BEGIN SELECT 1 END ELSE BEGIN SELECT 0 END";
                    SqlCommand command = new SqlCommand(query, connection);
                    connection.Open();
                    SqlDataReader reader = command.ExecuteReader();
            ...
            if (!incrementError)
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    try
                    {
                        ...
                        using (SqlCommand command = new SqlCommand("Insert_InventoryAdjustment_v2", connection))
                        {
                    ...
                    try
                    {
                        ...
                        if (!insertError && this.Text.Contains("<NEW>"))
                        {
                            ...
                            {
                                command.CommandType = CommandType.StoredProcedure;
                                command.Parameters.AddWithValue("@NumberingID", "IA");
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                try
                {
                    ...
                    if (reader.HasRows)
                    {
                        ...
                        oldSubItem.Text = reader.GetValue(2).ToString();
                        oldLot.Text = reader.GetValue(3).ToString();
                        oldSource.SelectedIndex = oldSource.FindString(reader.GetValue(4).ToString());
                        oldStage.SelectedIndex = oldStage.FindString(reader.GetValue(5).ToString());
                        oldForm.SelectedIndex = oldForm.FindString(reader.GetValue(6).ToString());
                        oldCropYear.SelectedIndex = oldCropYear.FindString(reader.GetValue(7).ToString());
                        oldGrade.SelectedIndex = oldGrade.FindStringExact(reader.GetValue(8).ToString());
                        oldArea.SelectedIndex = oldArea.FindString(reader.GetValue(9).ToString());
                        oldColor.SelectedIndex = oldColor.FindString(reader.GetValue(10).ToString());
                        oldFermentation.SelectedIndex = oldFermentation.FindString(reader.GetValue(11).ToString());
                        oldLength.SelectedIndex = oldLength.FindString(reader.GetValue(12).ToString());
                        oldProcess.SelectedIndex = oldProcess.FindString(reader.GetValue(13).ToString());
                        oldStalkPosition.SelectedIndex = oldStalkPosition.FindString(reader.GetValue(14).ToString());


                        oldWeightReceive.Text = reader.GetValue(17).ToString();
                        oldWeightTare.Text = reader.GetValue(18).ToString();
                        oldWeightNetto.Text = reader.GetValue(19).ToString();

                        oldWeightUoM.Text = reader.GetValue(20).ToString();
                        oldRemark.Text = reader.GetValue(21).ToString();

                        dr["DocumentID"] = reader.GetValue(0).ToString();
                ...
                if (response.Status == "Released")
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(ConnectionString))
                    {
                        try
                        {
                            ...
                            {
                                using (SqlCommand command = new SqlCommand("Update_InventoryAdjustmentDetail_Cost", connection))
                                {
                                    command.CommandType = CommandType.StoredProcedure;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Masukkan Madatory Remark", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }

        private void btnAcumatica_Click(object sender, EventArgs e)
        {
            this.Text = $"Universal Leaf [{Warehouse.Descr}] - Inventory Adjustment Process [{DocNumber}] - Syncing with Acumatica, please wait!";
            ...
            inventoryIssue.Description = $"{tbWarehouse.Text} Inventory Adjustment Transaction Issue";
            ...
            ////document details
            foreach (DataRow row in dtDetailIssue.Rows)
            {
                //check if doc buying already synced
                if (docNbr != row[24].ToString())
                {
                    docNbr = row[24].ToString();
                    if (!checkDocumentSync(docNbr))
                    {
                        MessageBox.Show($"Documents {docNbr} for bale is not synced to Acumatica!\nPlease sync old your documents first", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        allSynced = false;
                    }
                }

                ULTInventoryIssueDetail issueDetail = new ULTInventoryIssueDetail();
                issueDetail.InventoryID = row[1].ToString();
                issueDetail.Branch = docBranch;
                issueDetail.Location = AcumaticaCred.AcumaticaInvLocation;
                issueDetail.Subitem = row[2].ToString().Replace(".", "");
                issueDetail.Quantity = Convert.ToDecimal(row[19]);
                issueDetail.Description = row[3].ToString();
                issueDetail.Warehouse = tbWarehouse.Text;
                issueDetail.UOM = row[20].ToString();
            ...
            inventoryReceipt.Description = $"{tbWarehouse.Text} Inventory Adjustment Transaction Receipt";
            ...
            if (tbAcumaticaIssue.Text != "")
            {
                ...
                foreach (DataRow row in dtDetailRecipt.Rows)
                {
                        ...
                        using (SqlConnection connection = new SqlConnection(ConnectionString))
                        {
                            try
                            {
                                ...
                                foreach (DataRowView rowView in dv_filter)
                                {
                                    ...
                                    using (SqlCommand command = new SqlCommand("Update_InventoryAdjustmentDetail_Sync", connection))
                                    {
                                        command.CommandType = CommandType.StoredProcedure;
        ...
        {
            oldWeightReceive.Text = tbScale.Text;
        }
        ...
        private void btnPrintDoc_Click(object sender, EventArgs e)
        {
                ...
                try
                {
                    ...
                    string query = $@"SELECT
                                        ...
                                        FROM
	                                        InventoryAdjustmentDetail
	                                    WHERE
		                                    InventoryAdjustmentDetail.DocumentID = '{DocNumber}'
                                        ORDER BY
	                                        right(LotNbr,5) DESC, Iostock DESC";
                    ...
                    SqlDataAdapter da = new SqlDataAdapter(command);
                    // prit check lagi
                    da.Fill(myDoc.InventoryAdjustmentDetail);
            ...
            InventoryAdjustmentPrint InventoryAdjustmentPrint = new InventoryAdjustmentPrint
            {
                ...
                DocNumber = tbDocNumber.Text,
                DocType = "INVENTORY ADJUST",
                DocDate = tbDate.Text,
                DocStatus = tbStatus.Text,
                DocDetails = myDoc,
                Remark = remark.Text
            };
            InventoryAdjustmentPrint.ShowDialog();
        }
                ...
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    try
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            {
                                int index = cbLot.FindString(tbLot.Text);
        ...
        private void btnSaveLot_Click(object sender, EventArgs e)
        {
                ...
                if (radioButton1.Checked)
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(ConnectionString))
                    {
                        ...
                        using (SqlCommand command = new SqlCommand("Insert_InventoryAdjustment_v2_Detail", connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@LotNbr", $"{Math.Abs(FiscalInfo.CurrentFiscalYear) % 100}{Warehouse.WarehouseID}-");
                            command.Parameters.AddWithValue("@Source", oldSource.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Stage", oldStage.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Form", oldForm.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@CropYear", oldCropYear.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Grade", oldGrade.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Area", oldArea.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Color", oldColor.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Fermentation", oldFermentation.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Length", oldLength.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@StalkPosition", oldStalkPosition.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@Process", oldProcess.SelectedItem.ToString().Split('|')[0].Trim());
                            command.Parameters.AddWithValue("@WeightRope", Convert.ToDecimal(oldWeightReceive.Text));
                            command.Parameters.AddWithValue("@WeightShipping", "0");
                            command.Parameters.AddWithValue("@WeightReceive", Convert.ToDecimal(oldWeightReceive.Text));
                            command.Parameters.AddWithValue("@WeightTare", Convert.ToDecimal(oldWeightTare.Text));
                            command.Parameters.AddWithValue("@WeightNetto", Convert.ToDecimal(oldWeightNetto.Text));
                            command.Parameters.AddWithValue("@UoM", oldWeightUoM.Text);
                            command.Parameters.AddWithValue("@CostUnit", "0");
                            command.Parameters.AddWithValue("@Remark", oldRemark.Text);
                            command.Parameters.AddWithValue("@SyncDetail", 0);
                            command.Parameters.AddWithValue("@OldDocumentID", "");
                            command.Parameters.AddWithValue("@OldLotNbr", "");
                            command.Parameters.AddWithValue("@OldData", "");
                            command.Parameters.AddWithValue("@TypeAdjustment", 1);
                            command.Parameters.AddWithValue("@BuyerName", "");
                            ...
                            command.Parameters.AddWithValue("@Iostock", 1);
                            command.ExecuteNonQuery();
                ...
                else if (radioButton2.Checked)
                {
                    ...
                    if (dtEntry.Rows.Count > 0 && remark.Text.Trim() != null)
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(ConnectionString))
                        {
                            ...
                            string lotnew = $"{Math.Abs(FiscalInfo.CurrentFiscalYear) % 100}{Warehouse.WarehouseID}-{oldInventory.SelectedItem.ToString().Split('|')[0].Trim() ?? "XX"}{oldSource.SelectedItem.ToString().Split('|')[0].Trim() ?? "X"}{oldProcess.SelectedItem.ToString().Split('|')[0].Trim() ?? "xx"}{oldForm.SelectedItem.ToString().Split('|')[0].Trim() ?? "XX"}";
                            ...
                            using (SqlCommand command = new SqlCommand("Insert_InventoryAdjustment_v2_Detail", connection))
                            {
                                ...
                                command.Parameters.AddWithValue("@Stage", dtEntry.Rows[0]["Stage"].ToString());
                                command.Parameters.AddWithValue("@Form", dtEntry.Rows[0]["Form"].ToString());
                                command.Parameters.AddWithValue("@CropYear", dtEntry.Rows[0]["CropYear"].ToString());
                                command.Parameters.AddWithValue("@Grade", dtEntry.Rows[0]["Grade"].ToString());
                                command.Parameters.AddWithValue("@Area", dtEntry.Rows[0]["Area"].ToString());
                                command.Parameters.AddWithValue("@Color", dtEntry.Rows[0]["Color"].ToString());
                                command.Parameters.AddWithValue("@Fermentation", dtEntry.Rows[0]["Fermentation"].ToString());
                                command.Parameters.AddWithValue("@Length", dtEntry.Rows[0]["Length"].ToString());
                                command.Parameters.AddWithValue("@StalkPosition", dtEntry.Rows[0]["StalkPosition"].ToString());
                                command.Parameters.AddWithValue("@Process", dtEntry.Rows[0]["Process"].ToString());
                                command.Parameters.AddWithValue("@WeightRope", dtEntry.Rows[0]["WeightRope"].ToString());
                                command.Parameters.AddWithValue("@WeightShipping", dtEntry.Rows[0]["WeightShipping"].ToString());
                                command.Parameters.AddWithValue("@WeightReceive", dtEntry.Rows[0]["WeightReceive"].ToString());
                                command.Parameters.AddWithValue("@WeightTare", dtEntry.Rows[0]["WeightTare"].ToString());
                                command.Parameters.AddWithValue("@WeightNetto", dtEntry.Rows[0]["WeightNetto"].ToString());
                                command.Parameters.AddWithValue("@UoM", dtEntry.Rows[0]["UoM"].ToString());
                                command.Parameters.AddWithValue("@CostUnit", dtEntry.Rows[0]["CostUnit"].ToString());
                                command.Parameters.AddWithValue("@Remark", dtEntry.Rows[0]["Remark"].ToString());
                                command.Parameters.AddWithValue("@SyncDetail", "0");
                                command.Parameters.AddWithValue("@OldDocumentID", dtEntry.Rows[0]["DocumentID"].ToString());
                                command.Parameters.AddWithValue("@OldLotNbr", dtEntry.Rows[0]["LotNbr"].ToString());
                                command.Parameters.AddWithValue("@OldData", JsonConvert.SerializeObject(dtEntry));
                                command.Parameters.AddWithValue("@TypeAdjustment", 2);
                                command.Parameters.AddWithValue("@BuyerName", dtEntry.Rows[0]["BuyerName"].ToString());
                                command.Parameters.AddWithValue("@CreatedDate", DateTime.Now);
                                command.Parameters.AddWithValue("@EditDate", DateTime.Now);
                                command.Parameters.AddWithValue("@DeleteDate", DateTime.Now);
                                command.Parameters.AddWithValue("@operationType", 0);
                            ...
                            using (SqlCommand command = new SqlCommand("Insert_InventoryAdjustment_v2_Detail", connection))
                            {
                                ...
                                command.Parameters.AddWithValue("@StalkPosition", oldStalkPosition.SelectedItem.ToString().Split('|')[0].Trim());
                                command.Parameters.AddWithValue("@Process", oldProcess.SelectedItem.ToString().Split('|')[0].Trim());
                                command.Parameters.AddWithValue("@WeightRope", oldWeightReceive.Text);
                                command.Parameters.AddWithValue("@WeightShipping", "0");
                                command.Parameters.AddWithValue("@WeightReceive", oldWeightReceive.Text);
                                command.Parameters.AddWithValue("@WeightTare", oldWeightTare.Text);
                                command.Parameters.AddWithValue("@WeightNetto", oldWeightNetto.Text);
                                command.Parameters.AddWithValue("@UoM", oldWeightUoM.Text);
                                command.Parameters.AddWithValue("@CostUnit", dtEntry.Rows[0]["CostUnit"].ToString());
                                command.Parameters.AddWithValue("@Remark", oldRemark.Text);
                                command.Parameters.AddWithValue("@SyncDetail", 0);
                                ...
                                command.Parameters.AddWithValue("@OldLotNbr", dtEntry.Rows[0]["LotNbr"].ToString());
                                command.Parameters.AddWithValue("@OldData", JsonConvert.SerializeObject(dtEntry));
                                command.Parameters.AddWithValue("@TypeAdjustment", 4);
                                command.Parameters.AddWithValue("@BuyerName", dtEntry.Rows[0]["BuyerName"].ToString());
                                command.Parameters.AddWithValue("@CreatedDate", DateTime.Now);
                                command.Parameters.AddWithValue("@EditDate", DateTime.Now);
                                command.Parameters.AddWithValue("@DeleteDate", DateTime.Now);
                    ...
                    if (dtEntry.Rows.Count > 0 && remark.Text.Trim() != null)
                    {
                        ...
                        using (SqlConnection connection = new SqlConnection(ConnectionString))
                        {
                            ...
                            using (SqlCommand command = new SqlCommand("Insert_InventoryAdjustment_v2_Detail", connection))
                            {
                                ...
                                command.Parameters.AddWithValue("@OldLotNbr", dtEntry.Rows[0]["LotNbr"].ToString());
                                command.Parameters.AddWithValue("@OldData", JsonConvert.SerializeObject(dtEntry));
                                command.Parameters.AddWithValue("@TypeAdjustment", 3);
                                command.Parameters.AddWithValue("@BuyerName", dtEntry.Rows[0]["BuyerName"].ToString());
                                command.Parameters.AddWithValue("@CreatedDate", DateTime.Now);
                                command.Parameters.AddWithValue("@EditDate", DateTime.Now);
                                command.Parameters.AddWithValue("@DeleteDate", DateTime.Now);
            ...
            {
                MessageBox.Show("Masukkan type adjustment anda", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }

            oldWeightReceive.Text = "0.00";
            oldWeightTare.Text = "0.00";
            oldWeightNetto.Text = "0.00";
        ...
        private void htung_berat(object sender, EventArgs e)
        {
            if ((double.TryParse(oldWeightReceive.Text, out double n1) || int.TryParse(oldWeightReceive.Text, out int n)) && (int.TryParse(oldWeightTare.Text, out int asd) || double.TryParse(oldWeightTare.Text, out double dd2)))
            {
                double a = Convert.ToDouble(oldWeightReceive.Text) - Convert.ToDouble(oldWeightTare.Text);
                oldWeightNetto.Text = a.ToString("N2");
            }
        }

        private void hitung_berat(object sender, EventArgs e)
        {
            if ((double.TryParse(oldWeightReceive.Text, out double n1) || int.TryParse(oldWeightReceive.Text, out int n)) && (int.TryParse(oldWeightTare.Text, out int asd) || double.TryParse(oldWeightTare.Text, out double dd2)))
            {
                double a = Convert.ToDouble(oldWeightReceive.Text) - Convert.ToDouble(oldWeightTare.Text);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (dgvDetail.SelectedRows.Count > 0)
            {
                ...
                GenericLotPrintThermal lotPrint = new GenericLotPrintThermal
                {
                    ...
                    Date = tbDate.Text,
                    Remark = dgvDetail.Rows[dgvDetail.SelectedRows[0].Index].Cells[22].FormattedValue.ToString(),
                    Area = dgvDetail.Rows[dgvDetail.SelectedRows[0].Index].Cells[9].FormattedValue.ToString(),
                    QRImage = QRImage,
                    StrCrop = dgvDetail.Rows[dgvDetail.SelectedRows[0].Index].Cells[7].FormattedValue.ToString(),
                    Forms = dgvDetail.Rows[dgvDetail.SelectedRows[0].Index].Cells[6].FormattedValue.ToString()
                };
