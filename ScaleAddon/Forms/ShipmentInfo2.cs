...
namespace ScaleAddon.Forms
{
    public partial class ShipmentInfo2 : Form
    {
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    if (reader.HasRows)
                    {
                        ...
                        adjs.Text = reader.GetValue(20).ToString();
                    }
                }
                catch (Exception e)
                {
        ...
        private void saveLot()
        {
                ...
                try
                {
                    ...
                    string query = $@"SELECT *
                                        FROM
	                                        ShipmentInfoDetail
                                        WHERE
                                            DocumentID = '{DocNumber}' and
                                            WarehouseID = '{tbWarehouse.Text}' and
                                            InventoryID = '{dtEntry.Rows[0].ItemArray[1].ToString()}' and
                                            SubItem = '{dtEntry.Rows[0].ItemArray[2].ToString().Replace(".","")}'";
            ...
            if (dtEntrySave.Rows.Count > 1)
            {
                MessageBox.Show($"--Insert error! Terjadi SO line ganda dari data {DocNumber},{tbWarehouse.Text} ,{dtEntry.Rows[0].ItemArray[1].ToString()} dan {dtEntry.Rows[0].ItemArray[2].ToString()} , ", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else if (dtEntrySave.Rows.Count == 1)
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    try
                    {
                        ...
                        using (SqlCommand command = new SqlCommand("Insert_ShipmentInfoAllocation_C2", connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@SOLine", dtEntrySave.Rows[0].ItemArray[6].ToString());
            ...
            {
                MessageBox.Show($"--Insert error! SO line  dari data {DocNumber},{tbWarehouse.Text} ,{dtEntry.Rows[0].ItemArray[1].ToString()} dan {dtEntry.Rows[0].ItemArray[2].ToString()} tidak dapat di temukan , ", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
