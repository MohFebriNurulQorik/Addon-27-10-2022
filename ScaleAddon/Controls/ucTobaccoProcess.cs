...
{
    public partial class ucTobaccoProcess : UserControl
    {
        ...
        public ucTobaccoProcess()
        {
            InitializeComponent();
        }

        private void ucTobaccoProcess_Load(object sender, EventArgs e)
        {
            Dock = DockStyle.Fill;
            ParentForm.Text = $"Universal Leaf [{Warehouse.Descr}] - Tobacco Process List";
        ...
        private void LoadData()
        {
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                try
                {
                    string query = $"SELECT * from TobaccoProcess";
                    ...
                    //Header Text
                    dgvProcess.Columns[0].HeaderText = "Process Code";
                    dgvProcess.Columns[1].HeaderText = "Process Name";
                }
                catch (Exception e)
                {
        ...
        {
            LoadData();
            DataView dv_filter = new DataView(dtProcess, $"ProcessName LIKE '%{tbFilter.Text}%'", "ProcessCode Asc", DataViewRowState.CurrentRows);
            dgvProcess.DataSource = dv_filter;
        }
        ...
        private void btnAcumatica_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ParentForm.Text = $"Universal Leaf [{Warehouse.Descr}] - Tobacco Process List - Syncing with Acumatica, please wait!";
                ...
                var masterProcessApi = new ULTMasterProcessApi(configuration);
                var masterProcess = masterProcessApi.GetList();
                ...
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    foreach (var process in masterProcess)
                    {
                        try
                        {
                            ...
                            using (SqlCommand command = new SqlCommand("Insert_TobaccoProcess", connection))
                            {
                                command.CommandType = CommandType.StoredProcedure;
                                command.Parameters.AddWithValue("@ProcessCode", process.ProcessCode.Value);
                                command.Parameters.AddWithValue("@ProcessName", process.ProcessName.Value ?? "");
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    try
                    {
                        ...
                        string query = $@"TRUNCATE TABLE TobaccoProcess;";
